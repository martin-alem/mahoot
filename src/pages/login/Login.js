...
function Login(props) {
  ...
  React.useEffect(() => {
    if (Object.keys(params).length > 0 && params["code"] && params["state"]) {
      const url = "https://mahoot-auth-server-gvnig.ondigitalocean.app/api/v1/linkedin/auth";
      const method = "POST";
      const body = params;
      userActionContext.linkedInLogin(url, method, body);
    } else if (Object.keys(params).length > 0 && params["code"]) {
      const url = "https://mahoot-auth-server-gvnig.ondigitalocean.app/api/v1/github/auth";
      const method = "POST";
      const body = params;
